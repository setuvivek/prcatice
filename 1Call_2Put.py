...
for i in x:
    for j in x[i]:
        if j['optionType'] == 'Call':
            call_op.append({'underlying': j['underlying'], 'identifier': j['identifier'], 'instrumentType': j['instrumentType'],
                   'instrument': j['instrument'], 'expiryDate': j['expiryDate'],
                   'optionType': j['optionType'], 'strikePrice': j['strikePrice'], 'lastPrice': j['lastPrice'],
                   'pChange': j['pChange'],
                   'openPrice': j['openPrice'], 'highPrice': j['highPrice'],
                   'lowPrice': j['lowPrice'], 'numberOfContractsTraded': j['numberOfContractsTraded'],
                   'totalTurnover': j['totalTurnover'], 'premiumTurnover': j['premiumTurnover'],
                   'openInterest': j['openInterest'], 'underlyingValue': j['underlyingValue']})
        if j['optionType'] == 'Put':
            put_op.append({'underlying': j['underlying'], 'identifier': j['identifier'], 'instrumentType': j['instrumentType'],
                     'instrument': j['instrument'], 'expiryDate': j['expiryDate'],
                     'optionType': j['optionType'], 'strikePrice': j['strikePrice'], 'lastPrice': j['lastPrice'],
                     'pChange': j['pChange'],
                     'openPrice': j['openPrice'], 'highPrice': j['highPrice'],
                     ...
                     'totalTurnover': j['totalTurnover'], 'premiumTurnover': j['premiumTurnover'],
                     'openInterest': j['openInterest'], 'underlyingValue': j['underlyingValue']})
df1 = [print(i) for i in call_op]
print()
df2 = [print(i) for i in put_op]

import pandas as pd

# Create a Pandas dataframe from the data.
df = pd.DataFrame(call_op)
df3 = pd.DataFrame(put_op)
# Create a Pandas Excel writer using XlsxWriter as the engine.
writer = pd.ExcelWriter('/home/setu/Documents/call_put.xlsx', engine='xlsxwriter')
