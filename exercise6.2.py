...
workbook = xlsxwriter.Workbook('output/ex6.xlsx')
callsheet = workbook.add_worksheet('call')
putsheet = workbook.add_worksheet('put')
x = {
    "value": [
        {
...
head = ['underlying', 'identifier', 'instrumentType', 'instrument', 'expiryDate', 'optionType', 'strikePrice',
...
header = 0
for i in head:
    callsheet.write(0, header, i)
    putsheet.write(0, header, i)
    header += 1
...
for i in x:
    for j in x[i]:
        ...
        # temp=['underlying':underlying, 'identifier': identifier,'instrumentType':instrumentType,'instrument':instrument,'expiryDate':expiryDate,'optionType':optionType,'strikePrice':strikePrice,'lastPrice':lastPrice,'pChange':pChange,'openPrice':openPrice, 'highPrice': highPrice,'lowPrice':lowPrice,'numberOfContractsTraded':numberOfContractsTraded,'totalTurnover': totalTurnover,'premiumTurnover':premiumTurnover,'openInterest':openInterest,'underlyingValue':underlyingValue]
        ...
        if optionType == 'Put':
            # put.append({'underlying':underlying, 'identifier': identifier,'instrumentType':instrumentType,'instrument':instrument,'expiryDate':expiryDate,'optionType':optionType,'strikePrice':strikePrice,'lastPrice':lastPrice,'pChange':pChange,'openPrice':openPrice, 'highPrice': highPrice,'lowPrice':lowPrice,'numberOfContractsTraded':numberOfContractsTraded,'totalTurnover': totalTurnover,'premiumTurnover':premiumTurnover,'openInterest':openInterest,'underlyingValue':underlyingValue})
            sheet = putsheet
        elif optionType == 'Call':
            # call.append([{'underlying': underlying, 'identifier': identifier, 'instrumentType': instrumentType,'instrument': instrument, 'expiryDate': expiryDate, 'optionType': optionType,'strikePrice': strikePrice, 'lastPrice': lastPrice, 'pChange': pChange, 'openPrice': openPrice,'highPrice': highPrice, 'lowPrice': lowPrice, 'numberOfContractsTraded': numberOfContractsTraded,'totalTurnover': totalTurnover, 'premiumTurnover': premiumTurnover, 'openInterest': openInterest,'underlyingValue': underlyingValue}])
            sheet = callsheet
        else:
            continue
        for t in temp:
            if sheet == callsheet: row = rowcall
            if sheet == putsheet: row = rowput
            sheet.write(row, col, t)
            col += 1
        if sheet == callsheet: rowcall += 1
        if sheet == putsheet: rowput += 1
        # output.append(temp)
