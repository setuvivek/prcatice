...
for i in mylist1:
    ...
    if warehouse in dict:
        list_a = dict[warehouse]
        list_a.append({'product': i.get('product'), 'quantity': i.get('quantity')})
    else:
        dict.update({warehouse: [{'product': i.get('product'), 'quantity': i.get('quantity')}]})
print(dict)
dict1 = {}
warehouse = input("Enter Warehouse name : ")
product = input("Enter Product : ")
quantity = float(input("Enter quantity : "))
dict1[warehouse] = [{'product':product , 'quantity': quantity}]
print(dict1)

if warehouse in dict.keys():
    dict[warehouse].append({'product': product, 'quantity': quantity})
    print(dict[warehouse])
else:
    dict[warehouse] = [{'product': product, 'quantity': quantity}]
    print(dict)
