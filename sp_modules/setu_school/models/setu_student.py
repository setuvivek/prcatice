...
#------------------------------------------------
    ...
    def create(self, vals):
        rec = self.env['setu.teacher'].search([('class_teacher', '=', 'True'), ('standard_id', '=', vals.get('standard_id')),('medium_id', '=', vals.get('medium_id')),('division_id', '=', vals.get('division_id'))], limit=1)
        if rec:
    ...
    def check_not_null_name(self):
        for rec in self:
            if not (rec.first_name and rec.standard_id and rec.division_id and rec.medium_id):
                raise ValidationError("Enter Name,Standard,Division,Medium")

    def assign(self):
        rec =self.env['setu.teacher'].search([('class_teacher','=','True'),('standard_id','=',self.standard_id.id),('medium_id','=',self.medium_id.id),('division_id','=',self.division_id.id)],limit=1)
